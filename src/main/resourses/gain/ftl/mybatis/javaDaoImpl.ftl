...
public class ${voClassName}DaoImpl extends GenericDaoImpl implements ${voClassName}Dao{
    ...
    public ${voClassName} get${voClassName}ById(<#assign i=0><#list columnList as column><#if column.key><#assign i=i+1><#if (i>1)>,</#if>${column.propertyType} ${column.propertyName}</#if></#list>) {
        ...
        return this.selectOneFromWriter("${packageName?replace('/','.')}.model.${mypackageName}.${voClassName}Mapper.get${voClassName}ById",keyMap);
        <#else>
            <#list columnList as column>
                <#if column.key>
        return this.selectOneFromWriter("${packageName?replace('/','.')}.model.${mypackageName}.${voClassName}Mapper.get${voClassName}ById",${column.propertyName});
                </#if>
            </#list>
        </#if>
    }
    ...
    public List<${voClassName}> get${voClassName}List(${voClassName} ${parmString}) {
        return this.selectList("${packageName?replace('/','.')}.model.${mypackageName}.${voClassName}Mapper.get${voClassName}List",${parmString});
    }
