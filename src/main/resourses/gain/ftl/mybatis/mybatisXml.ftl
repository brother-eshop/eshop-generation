...
<mapper namespace="${packageName?replace('/','.')}.model.${mypackageName}.${voClassName}Mapper">
    ...
    <sql id="${tableName}_where_conditions">
        ...
        <if test="${column.propertyName} != null">
        	${column.columnName}=#${"{"}${column.propertyName}${"}"} AND 
        </if>
        </#list>
        (status!=2)
        </where>
    ...
    <select id="get${voClassName}List" resultMap="${voClassName}Result" parameterType="${packageName?replace('/','.')}.model.${mypackageName}.${voClassName}">
        select
        <include refid="${tableName}_columns"/>
