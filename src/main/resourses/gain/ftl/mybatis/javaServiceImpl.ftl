...
import com.eshop.cache.util.QueryProvider;
/**
 * ${voClassName}管理接口
 * User: spencer
...
public class ${voClassName}ServiceImpl implements ${voClassName}Service{
    ...
    public Long add${voClassName}(${voClassName} ${parmString}){
    	...
    	if(res>0){
    		queryCache.setObj("SNS_${voClassName}",String.valueOf(${parmString}.getId()),${parmString});
    	}
    	return res;
    }
...
<#list columnList as column>
    ...
    public Long delete${voClassName}ById(<#assign i=0><#list columnList as column><#if column.key><#assign i=i+1><#if (i>1)>,</#if>${column.propertyType} ${column.propertyName}</#if></#list>){
    	queryCache.deleteObj("SNS_${voClassName}",String.valueOf(<#assign i=0><#list columnList as column><#if column.key><#assign i=i+1><#if (i>1)></#if>${column.propertyName}</#if></#list>));
	...
	public Long delete${voClassName}ByObj(${voClassName} ${parmString}){
    	queryCache.deleteObj("SNS_${voClassName}",String.valueOf(${parmString}.getId()));
    ...
    public Long delete${voClassName}ByIdList(List<<#assign i=0><#list columnList as column><#if column.key><#assign i=i+1><#if (i>1)>,</#if>${column.propertyType} </#if></#list>> ids){
    	for(<#assign i=0><#list columnList as column><#if column.key><#assign i=i+1><#if (i>1)>,</#if>${column.propertyType} </#if></#list> id:ids){
    		queryCache.deleteObj("SNS_${voClassName}",String.valueOf(id));
    	}
    ...
    public Long update${voClassName}ByObjAndConditions(${voClassName} s,${voClassName} c){
    	queryCache.deleteObj("SNS_${voClassName}",String.valueOf(s.getId()));
...
<#list columnList as column>
    ...
    public ${voClassName} get${voClassName}ById(<#assign i=0><#list columnList as column><#if column.key><#assign i=i+1><#if (i>1)>,</#if>${column.propertyType} ${column.propertyName}</#if></#list>){
    	...
    	if(${parmString}==null){
    		...
    		if(${parmString}!=null){
    			queryCache.setObj("SNS_${voClassName}",String.valueOf(<#assign i=0><#list columnList as column><#if column.key><#assign i=i+1><#if (i>1)>,</#if> ${column.propertyName}</#if></#list>),${parmString});
    		}
    	}
    	return ${parmString};
    }
    ...
    public List<${voClassName}> get${voClassName}List(${voClassName} ${parmString}){
    	List<Long> ids =  ${parmString}Dao.get${voClassName}IdList(${parmString});
